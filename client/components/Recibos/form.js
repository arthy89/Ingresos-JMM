...
const Form = forwardRef(
  ({ save, isEdit, id, onClose, ult, conceptos, est_data }, ref) => {
    ...
    // End Point
    const endpoint = isEdit ? `api/recibos/${id}?_method=PUT` : "api/recibos";
    ...
    const buscar_est = async () => {
      try {
        est_found_ref.current = await EstudianteService.getDNI(
          form.data.estudiante.dni,
        );
        ...
        <form ref={ref} onSubmit={onSave} className="flex flex-col gap-2">
          ...
          <ModalBody>
            ...
            <Items
              ...
              conceptos={conceptos}
              certificado={
                est_found_ref.current?.certificado || est_data?.certificado
              }
            />
